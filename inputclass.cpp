...
float InputClass::NormaliseValue(float Rotation)
{
	if (Rotation <= -60) return -60;
	if (Rotation >= 60) return 60;
...
bool InputClass::IsEnterReleased()
{
	if (m_keyboardState[DIK_RETURN] & 0x80)
	{
		prevEscapePressed = true;
		return false;
	}
	if (!(m_keyboardState[DIK_RETURN] & 0x80)&& prevEscapePressed)
	{
		prevEscapePressed = false;
		return true;
	}
